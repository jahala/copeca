...
If the project has no features yet (personas defined via `/tend position`, but no features), the engine routes to `/tend brainstorm` — define your first feature; the unbound-spine warnings (unreferenced personas, orphan opportunities) are expected until the first feature exists, so surface the brainstorm action and stop.

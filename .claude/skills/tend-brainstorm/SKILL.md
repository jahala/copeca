...
- **Every check has a `validates` slot.** Ask, per check: "Which slot claim does this check anchor to?" Pick one of `what` / `why` / `impact` / `fit` / `where` / `how`. The slot pin keeps checks honest - no orphan checks that prove nothing in particular. Note: `what` / `impact` / `how` are the checkable slots (output-bearing claims); `why` / `fit` / `where` are justification / placement / code-location and are not required to have a check — but if a why/fit/where claim is *falsifiable* rather than contextual, add a check for it anyway.
...
- **A content artifact verifies presence + accuracy, not a passing test.** When the feature *is* a delivered document — a roadmap, a landing / marketing page, a README, a design doc — there's no behaviour to synthetic-test. Its checks use `code-review` / `user-report` / `agent-judgment` (is the claim present? accurate? published?), `smoke` is a logged "no smoke — content artifact", and the slots describe the artifact's job, not a capability's. This is guidance, not a schema kind: the polyglot shape is unchanged; only the verification surface differs.
...
- **Log decisions**: When the user picks an approach, makes a scoping choice, or decides to skip a smoke for a library-shaped feature, write to `decisions[]` via `tend_update_feature`. Each entry needs `id`, `at`, `decision`, `rationale`. Include `alternatives_considered` when multiple options were evaluated. Deciding NOT to build something, or rejecting an approach, is a `decisions[]` entry — not just a chat reply. A scope-out that isn't recorded gets rebuilt by the next agent.
- After completion: single-sentence summary unless user requests more.

## Orient first

**Before you change anything**: call `tend_get_context(<id>)` if a feature already exists, read `decisions[]` (don't re-litigate settled calls), and skim `git log -- docs/tend/features/<id>.tend.html <coverage_files>` to understand how it got here. For a brand-new feature, orient on the garden: `bash docs/tend/overview.html data | jq '.catalog'`.
...
**The `journey_phase` field is the litmus test.** If you cannot assign one of the project's `catalog.journey_phases` (a journey moment a persona's job triggers) to the concept without stretching, it is not a top-level feature.
...
Journey phases come from `/tend position` too — derived from the personas'
`jobs[].when` moments, not a fixed set. If `catalog.journey_phases` is unset,
the redirect above already covers it (position sets them); don't seed a
universal six here.
...
`verify` regex). The `smoke.cmd` must exercise at least one of the
feature's checks/tests — a smoke that runs nothing real is theatre. For
features with no runtime to smoke-test, log the decision in `decisions[]`
and move on.

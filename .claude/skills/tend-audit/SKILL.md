...
**Before you change anything**: call `tend_get_context(<id>)`, read `decisions[]` (don't re-litigate settled calls), and skim `git log -- docs/tend/features/<id>.tend.html <coverage_files>` to understand how it got here. When committing audit findings, add `Tend-Feature: <id>` as a git trailer so the audit commit maps to the feature (`git log --grep 'Tend-Feature:.*<id>'`).
...
Run `tend_validate`. A `coverage_path_missing` error (a `coverage_files[]` path that no longer resolves on disk, or lives outside the project's configured `source_dirs`) is a **config / claim fix, not an audit-verdict failure** — auditing the code cannot repair a stale path. Surface it in your notes and route it to `/tend change <feature>` (fix the path) or `tend_update_catalog({ changes: { source_dirs } })` (widen the roots); do **not** demote any check or `audit.result` because of it. The unclaimed file count from the coverage block is likewise informational and never blocks a `pass`.

### 8. Compute audit.result

- `pass`: every check has `verdict: pass`. Smoke (if defined) passed. (A `coverage_path_missing` error does not block a `pass` — it is a config/claim fix; see step 7.)
- `partial`: at least one check is `partial`, none are `fail`, smoke (if defined) passed.
- `fail`: any check is `fail`, OR smoke failed.

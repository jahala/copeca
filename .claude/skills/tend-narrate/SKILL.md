...
Under parallel batch dispatch (one subagent per polyglot), a non-interactive
subagent cannot block on a human here: it returns its proposed section list
and lede to the dispatching orchestrator, which gates approval for the wave
(or surfaces it to the user) before the subagents draft the bodies.
...
MCP validates, splices, refreshes denorm, returns the new state. The
garden is structurally a feature: write its `narrative` / `dek` / `media`
(and its bet slots) the same way, with `id: "garden"` — there is no
separate garden-write tool. `tend_update_catalog` is only for the garden's
`catalog` (personas / opportunities / journey_phases).

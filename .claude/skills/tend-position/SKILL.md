...
**jobs[]**), `catalog.opportunities[]` (id, description, value,
confidence, source, signals, **`personas: string[]`**), and
**`catalog.journey_phases`** (the project's journey moments, derived from the
personas' job situations) on the garden polyglot.
...
### Journey phases — derive them from the jobs

The project's **journey phases are not a fixed universal set** — they are the
recurring *moments* the personas are in, read straight from the `When [situation]`
clause of every job you just wrote. Collect the `jobs[].when` situations across
all personas, cluster the ones that are the same moment, and order them into the
project's journey. That ordered set is `catalog.journey_phases`.

- A dev tool's personas (people building software) yield something like
  `orient · shape · plan · orchestrate · verify · review` (tend's own set —
  note there is no `build` phase: tend *directs* the build via adapters rather
  than performing it, so no persona has a "build" moment).
- A shopping app's yield `browse · compare · checkout · track`.
- A compliance tool's yield `intake · review · attest · export`.

Derive from *these* personas; never stamp a default. Name each phase as the
user's felt moment (a verb or short noun), **not a tend skill name**. A phase with
no persona-moment behind it doesn't belong; a recurring moment with no phase is a
missing one. Write the set in step 7's `tend_update_catalog` call (`journey_phases:
[...]`); downstream, `/tend brainstorm` and `/tend discover` assign each feature
one of these phases and `journey_phase_valid` honours exactly this list.

**The phases are what give each persona a legible journey.** A persona's rail
renders their *journey through the product* — the features bound to them
(`bound_features`), grouped by `journey_phase` and ordered by the project's
`journey_phases`: where they enter, which features they touch at each moment,
where they exit. This is **derived, never authored** — the snapshot side-effect
groups and orders it for you. You don't write a persona's journey; you write
good phases and bind features well, and the journey falls out. So a persona whose
features all land in one phase reads as a one-stop journey; spread a persona's
features across the phases and their rail tells the whole arc. The union of every
persona's phased features is exactly the project's `journey_phases` — if a phase
appears in no persona's journey, it has no one living it (`journey_phase_unused`).
...
Pass `force: true` to switch to wholesale replace (for intentional deletion or reorder) — referenced entries are still never droppable, even with force.

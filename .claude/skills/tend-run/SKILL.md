---
name: tend-run
description: Take a feature from plan to execution. The host agent can run the steps directly (you are already an agent — execute each step's Test then Implement with your own tools, marking status as you go), or hand the build to the polyglot via `bash <file>.tend.html run`, or emit an execution plan via an adapter (Claude Tasks, Markdown, GitHub Issues, Makefile) for an external orchestrator. Updates step status during execution; never writes `audit.result` or `status=verified`.
user-invocable: false
---

# Run: Steps -> Execution

Take a feature from plan to execution. Three paths depending on what's
appropriate for the project:

| Path                  | When                                                                                                       |
|-----------------------|------------------------------------------------------------------------------------------------------------|
| **Host agent runs directly** | The common case — you, the agent in this session, ARE the orchestrator. Read `steps[]` and execute each one's `Test:` → `Implement:` with your own tools (the PRE → RED → IMPL → GREEN → POST → VERIFY → DOC loop below), marking `steps[].status` via `tend_update_feature` as you go. No sub-shell, no adapter — your context already holds the feature. |
| **Polyglot run**      | Hand the build to a fresh single agent, detached from this session: `bash <file> run` iterates `steps[]` and invokes an agent CLI (`claude -p`) per step, then runs smoke. |
| **External adapter**  | Multi-agent or external orchestrator. `tend_output_execution_plan` emits the plan in the adapter's native format (Claude Tasks bundle, Markdown spec, GitHub Issues batch, Makefile); the orchestrator consumes it. |
...
**Before you change anything**: call `tend_get_context(<id>)`, read `decisions[]` (don't re-litigate settled calls), and skim `git log -- docs/tend/features/<id>.tend.html <coverage_files>` to understand how it got here.
...
- **Tend-Feature trailer on commits.** When committing feature work, add `Tend-Feature: <id>` as a git trailer line so commits map cleanly to features (`git log --grep 'Tend-Feature:.*<id>'` retrieves the full build history).
...
how do you want to execute <id>?
  ...
  makefile       — emit a GNU Makefile (one target per step) for make / CI
```
...
tend_output_execution_plan({
  feature_id: "<id>",
  adapter: "claude-tasks" | "markdown" | "github-issues" | "makefile"
})
```
...
- **makefile**: a GNU Makefile with one target per step (agent steps invoke the agent CLI on the step prompt; command/worktree steps run their command), dependency edges wired as Make prerequisites, plus a final smoke target (if `smoke.cmd` is set) that depends on every preceding target. Output: `docs/tend/output/makefile/Makefile` (+ per-step prompt files).
...
```bash
make -C docs/tend/output/makefile
```

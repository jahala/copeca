...
- **Scope-outs are first-class decisions.** Deciding NOT to make a change, or ruling out an approach during this change request, is a `decisions[]` entry (feature-level) or the garden's `decisions[]` (project-wide) — not just a chat reply. A scope-out that isn't recorded gets rebuilt by the next agent.

## Orient first

**Before you change anything**: call `tend_get_context(<id>)`, read `decisions[]` (don't re-litigate settled calls), and skim `git log -- docs/tend/features/<id>.tend.html <coverage_files>` to understand how it got here. A decision already logged is settled — don't re-litigate it without explicit user direction.

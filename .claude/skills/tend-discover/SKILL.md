...
Before enumerating anything, apply this filter to every candidate concept. A discovered concept belongs at the **top level** only when a persona performs a named journey moment at its surface (one of the project's `catalog.journey_phases` — a moment a persona's job triggers, not a fixed universal verb set) and there is a user-visible entry and exit condition. Anything else belongs as a **subpage** of the user-facing surface it implements.
...
- **`journey_phase`** - the moment a feature serves in *this project's* user
  journey. **The phases are project-specific and persona-derived — there is NO
  fixed universal set.** They live in `catalog.journey_phases`. Because discover
  usually runs before `/tend position` has planted personas, you normally
  **leave `journey_phase` unset here** (and `catalog.journey_phases` empty):
  `journey_phase_valid` is silent on an empty list, and `/tend position` derives
  the real phases from the personas' `jobs[].when` moments. If you make a
  provisional best-guess from the code surface, mark it best-guess and route the
  user to `/tend position` for the real journey. (Builder/dev-tool surfaces often
  resemble `orient/shape/plan/build/orchestrate/verify/review`; a shopping app
  `browse/compare/checkout/track` — derive from the actual personas, don't assume.)
...
- `journey_phases` - **derive from the personas if any exist** (cluster their
  `jobs[].when` situations into the project's journey moments); otherwise **leave
  empty** and let `/tend position` set them. Do NOT seed a fixed universal set —
  phases are project-specific (a dev tool's differ from a shopping app's).

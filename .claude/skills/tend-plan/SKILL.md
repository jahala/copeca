...
- **Log decisions**: When a non-obvious choice is made (architecture, library, approach, trade-off), write it to `decisions[]` via `tend_update_feature`. Each entry needs `id`, `at`, `decision`, `rationale`. Include `alternatives_considered` when multiple options were evaluated. Deciding NOT to build something, or ruling out an approach, is a `decisions[]` entry — not just a chat reply. A scope-out that isn't recorded gets rebuilt by the next agent.
